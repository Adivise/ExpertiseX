...
module.exports = { 
	...
	run: async (client, message, args, prefix) => {
        const msg = await message.channel.send(`*\`Turning on\`* **${client.commands.get('rate').config.name}** *\`This may take a few seconds...\`*`);
        ...
        await delay(5000);
        return msg.edit(`\`🔩\` | *Rate set to:* \`${args[0]}x\``).then(msg => {
			setTimeout(() => msg.delete(), 5000)
		});
	}
};