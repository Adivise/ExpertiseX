...
module.exports = {
    ...
    run: async (client, message, args) => {
        ...
        if (res.type === "PLAYLIST") {
            ...
            return message.reply({ content: `**Skipped • [${res.playlistName}](<${args}>)** \`${convertTime(player.queue.durationLength, true)}\` (${res.tracks.length} tracks)` })
        } else {
            player.play(new KazagumoTrack(res.tracks[0].getRaw(), message.author));

            return message.reply({ content: `**Skipped • [${res.tracks[0].title}](<${res.tracks[0].uri}>)** \`${convertTime(res.tracks[0].length, true)}\`` })
        }
    }
}
