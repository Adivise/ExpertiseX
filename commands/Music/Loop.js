    ...
    run: async (client, message, args, prefix) => {
		...
		if (!args[0] || args[0].toLowerCase() == 'current') {
			if (player.trackRepeat === false) {
				...
				return msg.edit(`\`🔁\` | *Song is loop:* \`Current\``).then(msg => {
				...
				return msg.edit(`\`🔁\` | *Song is unloop:* \`Current\``).then(msg => {
					setTimeout(() => msg.delete(), 5000)
				});
			}
		}
			...
			if (player.queueRepeat === true) {
				...
				return msg.edit(`\`🔁\` | *Song is unloop:* \`All\``).then(msg => {
					setTimeout(() => msg.delete(), 5000)
				});
			}
			else {
				...
				return msg.edit(`\`🔁\` | *Song is loop:* \`All\``).then(msg => {
					setTimeout(() => msg.delete(), 5000)
				});
			}
		}
