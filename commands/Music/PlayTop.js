...
module.exports = {
    ...
    run: async (client, message, args) => {
        ...
        if (res.type === "PLAYLIST") {
            ...
            return message.reply({ content: `**Shifted • [${res.playlistName}](<${args}>)** \`${convertTime(player.queue.durationLength, true)}\` (${res.tracks.length} tracks)` })
        } else {
            player.queue.add(res.tracks[0]);

            Normal(player);

            return message.reply({ content: `**Shifted • [${res.tracks[0].title}](<${res.tracks[0].uri}>)** \`${convertTime(res.tracks[0].length, true)}\`` })
        }
    }
}
